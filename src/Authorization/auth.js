let Login = new Lure.Content({
    ...
    Content: `<div class="authentication">
                  <div class="logo"></div>
                    ...
                    <input class="textbox textbox-pass"  id="auth-pass"  type="password" placeholder="Пароль" value="test">
                    ...
                    <div class="none" id="error"><span>Неправильный логин или пароль</span></div>
                </div>`,
        ...
        api.User_PasswordRestore(token, {
            Then: (r)=>{
                if (r > 0)
                {
                    ...
                    Lure.Pop.Notice('Не удалось восстановить пароль');
                    this.Show();
                }
            },
            Catch: ()=>{
        ...
        this.PwdRestore = new Lure.Content({
            ...
            Methods: function(){
                this.RestoreSend = function () {
                    ...
                    if (Email === '')
                        return Lure.Pop.Warn('Введите адрес электронной почты');
                    Lure.Button.Lock(this.ButtonSendRestore);
                    api.User_PasswordRestore_GetToken(Email, {
                        Then: (res)=>{
                            if (res === -3){
                                Lure.Pop.Notice('Пользователь с таким Email не найден')
                            }
                            else if (res === -2){
                                Lure.Pop.Notice('Пользователь удален')
                            }
                            else if (res === -1){
                                Lure.Pop.Notice('Не удалось отправить на указанный адрес')
                            }
                            else if (res > 0){
                                Lure.Pop.Success('Ссылка восстановления пароля отправлена на указанный адрес')
                            }
                        },
                        Catch: (e)=>{
                            Lure.Pop.Error('Неизвестная ошибка сервера', {LogItem: e})
                        },
                        Finally: ()=>{
    ...
    Methods: function () {
        this.Login = function () {
            ...
            api.Login(Login, Pass, null)
                ...
                .catch(()=>{
                    Lure.Pop.Error('Неправильный логин или пароль');
                    Lure.Button.Release(this.ButtonLogin);
                })
        };
